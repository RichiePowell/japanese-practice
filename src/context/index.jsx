...
export class Provider extends Component {
  ...
  checkAnswer = (answer) => {
    ...
    if (
      ...
      this.setState((prev) => ({
        wrongAnswerDialogActive: this.state.showWrongAnswerDialog
          ? true
          : false,
        ...
        wrongAnswers: {
          ...prev.wrongAnswers,
          [prev.currentCharacter]: parseInt(
            prev.wrongAnswers[prev.currentCharacter]
          )
            ? prev.wrongAnswers[prev.currentCharacter] + 1
            : 1,
        },
      }));
      ...
      this.setState((prev) => ({
        ...
        correctAnswers: {
          ...prev.correctAnswers,
          [prev.currentCharacter]: parseInt(
            prev.correctAnswers[prev.currentCharacter]
          )
            ? prev.correctAnswers[prev.currentCharacter] + 1
            : 1,
        },
      }));
  ...
  startGame = () => {
    this.setState(
      ...
      this.loadNewCharacter
    ); // Set gameStart state to true and load a new character
  };
  ...
  endGame = () => {
    ...
    if (
      this.state.correctAnswersTotal === 0 &&
      this.state.wrongAnswersTotal === 0
    ) {
      this.clearStats();
    } else {
      this.toggleReport();
      ...
      this.playSound(
        this.state.wrongAnswersTotal > this.state.correctAnswersTotal
          ? "gameOverBad"
          : "gameOver"
      );
    }
  ...
  toggleInput = () =>
    this.setState((prev) => ({ keyboardMode: !prev.keyboardMode }));
  ...
  toggleKana = (kana, overrideSelected = null) => {
    ...
    if (
      (!newKanaSelection.includes(kana) && overrideSelected === null) ||
      (!newKanaSelection.includes(kana) && overrideSelected === 1)
    ) {
  ...
  changeAnswerTimer = (seconds) =>
    this.setState({ answerTimer: parseInt(seconds) });
  changeGameTimer = (seconds) =>
    this.setState({ gameTimer: parseInt(seconds) });
  toggleWrongAnswerDialog = () =>
    this.setState((prev) => ({
      showWrongAnswerDialog: !prev.showWrongAnswerDialog,
    }));
  hideWrongAnswerDialog = () =>
    this.setState({ wrongAnswerDialogActive: false });
  ...
  loadKana = () => {
    ...
    Object.keys(this.state.kana).forEach(
      (kana) =>
        (newCharacterSet = {
          ...newCharacterSet,
          ...this.state.kana[kana].characters,
        })
    );
  ...
  loadNewCharacter = () => {
    ...
    if (this.state.currentCharacter.length)
      shuffledCharacters.splice(
        shuffledCharacters.indexOf(this.state.currentCharacter),
        1
      );

    const character = shuffledCharacters.shift(); // Grab the first one
    const answer = characters[character]; // Grab the answer
    const answerPrintable = characterSetData.firstAnswerAsCorrection
      ? answer[0]
      : answer.join(" or "); // Make answer printable, join multiple answers with "or"
